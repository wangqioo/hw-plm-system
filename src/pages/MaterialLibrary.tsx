...
import { Search, Filter, Plus, ChevronRight, Star, ExternalLink, ArrowUpDown } from 'lucide-react';
import { clsx } from 'clsx';
import { materials as allMaterials, categoryLabels } from '../data/mockData';
...
export default function MaterialLibrary() {
  ...
  const [sortBy, setSortBy] = useState<'preferredRank' | 'name' | 'qualityLevel'>('preferredRank');

  const filtered = useMemo(() => {
    return allMaterials
      .filter(m => {
        const q = search.toLowerCase();
        const matchSearch = !q || m.name.toLowerCase().includes(q) || m.partNumber.toLowerCase().includes(q) ||
          m.manufacturer.toLowerCase().includes(q) || m.manufacturerPN.toLowerCase().includes(q);
        const matchCat = selectedCat === 'all' || m.category === selectedCat;
        const matchQuality = selectedQuality === 'all' || m.qualityLevel === selectedQuality;
        const matchStatus = selectedStatus === 'all' || m.status === selectedStatus;
        return matchSearch && matchCat && matchQuality && matchStatus;
      })
      .sort((a, b) => {
        if (sortBy === 'preferredRank') return a.preferredRank - b.preferredRank;
        if (sortBy === 'qualityLevel') return a.qualityLevel.localeCompare(b.qualityLevel);
      ...
      <div className="flex-1 min-w-0 flex flex-col gap-4">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-0.5">共 {filtered.length} 条物料</p>
          </div>
          <Link
            to="/entry"
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <select
              ...
              className="text-xs border border-gray-200 rounded-lg px-2 py-1.5 text-gray-600 focus:outline-none focus:border-[#F97316] cursor-pointer"
            >
              <option value="preferredRank">优选排名</option>
              <option value="qualityLevel">质量等级</option>
          ...
          <div className="flex flex-wrap gap-1.5">
            {categories.map(cat => (
              ...
              >
                {cat === 'all' ? '全部' : categoryLabels[cat as MaterialCategory]}
              </button>
            ))}
          </div>
        </div>
              ...
              <thead>
                <tr className="bg-gray-50 border-b border-gray-200">
                  <th className="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">
                    <div className="flex items-center gap-1 cursor-pointer hover:text-gray-700" onClick={() => setSortBy('preferredRank')}>
                  ...
                  >
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1">
                        {m.preferredRank <= 1 && <Star size={13} className="text-yellow-400 fill-yellow-400" />}
                        <span className="text-xs font-mono text-gray-500">#{m.preferredRank}</span>
                      </div>
                    </td>
                    <td className="px-4 py-3">
                      ...
                      <div className="text-xs text-gray-400 mt-0.5 flex items-center gap-1.5">
                        ...
                        <span>{m.manufacturer} {m.manufacturerPN}</span>
                      </div>
                    </td>
                    <td className="px-4 py-3">
                      <span className="text-xs text-gray-600 bg-gray-100 px-2 py-0.5 rounded">
                        {categoryLabels[m.category as MaterialCategory]}
                      </span>
                    </td>
                    <td className="px-4 py-3">
        ...
        <div className="w-80 flex-shrink-0 bg-white rounded-xl border border-gray-200 overflow-auto">
          ...
          <div className="p-4 space-y-4">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              <QualityBadge level={selected.qualityLevel} />
              <StatusBadge status={selected.status} />
              {selected.preferredRank <= 1 && (
            ...
            <div className="space-y-2">
              ...
              {[
                ...
                { label: '厂商料号', value: selected.manufacturerPN },
                { label: '类别', value: categoryLabels[selected.category as MaterialCategory] },
                { label: '优选排名', value: `#${selected.preferredRank}` },
                { label: '单价', value: selected.unitPrice != null ? `¥${selected.unitPrice}` : '-' },
                { label: '库存', value: selected.stock?.toLocaleString() ?? '-' },
                { label: '货期', value: selected.leadTime ? `${selected.leadTime} 天` : '-' },
              ...
              <div className="bg-gray-50 rounded-lg p-3 space-y-1.5">
                {Object.entries(selected.parameters).map(([k, v]) => (
                  <div key={k} className="flex justify-between text-xs">
                    ...
                    <span className="font-mono font-medium text-gray-700">{v}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="flex flex-col gap-2 pt-2">
              <button className="w-full flex items-center justify-center gap-2 bg-[#F97316] text-white py-2 rounded-lg text-xs font-medium hover:bg-orange-600 transition-colors cursor-pointer">
                <ExternalLink size={13} />
                查看数据手册
              </button>
              <button className="w-full flex items-center justify-center gap-2 border border-gray-200 text-gray-600 py-2 rounded-lg text-xs font-medium hover:bg-gray-50 transition-colors cursor-pointer">
                提交质量反馈
              </button>
            </div>
          </div>
